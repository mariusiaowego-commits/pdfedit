...
```
□ Git — add → commit → push（未测不推）
□ Git push 代理挂 — `git config --local remote.origin.pushurl https://github.com/mariusiaowego-commits/pdfedit.git`
□ README — 改动需要同步更新
□ 成品 — 放在 output/，不上传 Git
□ Wiki沉淀 — 踩坑记录同步到 hermes-base/concepts/pdfedit-pitfalls.md
□ 用户确认 — 展示最终结果
```

## 收尾文档
- Wiki — `hermes-base/concepts/pdfedit-pitfalls.md`
- README — 项目根目录
